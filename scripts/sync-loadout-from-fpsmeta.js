 ...
 * Obiettivo: ricreare il database Loadout con armi + accessori reali presi da
 * schede meta leggibili. Gli utenti potranno creare build personalizzate
 * scegliendo dagli accessori verificati visti su build reali.
 */
...
const STYLE_MAP = new Map([
  ...
  ['close range', 'Corto raggio'],
  ['sniper support', 'Supporto cecchino'],
...
function buildId(weaponId, style, sourceUrl) { return 'fpsmeta-' + slug(`${weaponId}-${style}-${sourceUrl}`).slice(0, 82); }
...
function parseFpsMetaText(text, sourceUrl) {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    for (let back = startIndex - 1; back >= Math.max(0, startIndex - 12); back--) {
      ...
      if (!styleLine && /long range|close range|sniper support|resurgence|mobility|versatile|meta/i.test(line)) {
        styleLine = line;
        continue;
      }
      if (isLikelyWeaponName(line)) {
        weaponName = line;
        break;
      }
    }
    ...
    builds.push({
      weaponId,
      weaponName,
      category: guessCategory(weaponName + ' ' + styleLine),
      style: normalizeStyle(styleLine),
      pairs: uniquePairs,
      sourceUrl
    });
...
function mergeBuildData(state, build) {
  ...
  if (!weapons.some(w => w.id === build.weaponId)) {
    weapons.push({
      ...
      note: 'Importato da build FPSMeta e tradotto in italiano.',
      stato: 'pubblico',
  ...
  build.pairs.forEach((pair, index) => {
    ...
    if (!attMap.has(attId)) {
      const att = {
        ...
        fonte: SOURCE,
        fonteUrl: build.sourceUrl,
        note: 'Accessorio trovato in una build FPSMeta reale.',
        stato: 'pubblico',
        updatedAt: todayStr,
    ...
    if (!compMap.has(compKey)) {
      const row = {
        ...
        fonte: SOURCE,
        fonteUrl: build.sourceUrl,
        note: 'Compatibilità vista in una build FPSMeta reale.',
        stato: 'pubblico',
        updatedAt: todayStr,
        codmunityOrder: index + 1
      };
  ...
  if (buildAccessories.length >= 3) {
    ...
    if (!builds.some(b => b.id === id)) {
      builds.unshift({
        ...
        categoria: build.category,
        armaId: build.weaponId,
        armaNome: build.weaponName,
        stile: build.style,
        accessori: buildAccessories.slice(0, 8),
        note: `Build importata da ${SOURCE}. Slot tradotti in italiano.`,
        stato: 'approvato',
        fonte: SOURCE,
        fonteUrl: build.sourceUrl,
        createdAt: nowIso(),
...
async function main() {
  const report = {
    startedAt: nowIso(),
    source: SOURCE,
    mode: 'fpsmeta-warzone-meta-replace-full-database',
    checkedUrls: [],
    importedBuilds: 0,
  ...
  try {
    ...
    for (const url of START_URLS.slice(0, LIMIT || START_URLS.length)) {
      try {
        ...
        console.log(`${url}: ${extracted.length} build FPSMeta`);
      } catch (error) {
    ...
    if (!unique.size) report.errors.push('Nessuna build FPSMeta trovata.');
  } finally {
