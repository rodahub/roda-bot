...
/**
 * Directly spawns the official CLASSIFICA LIVE and TOP FRAGGER graphics after
 * Discord is ready. This intentionally bypasses panel refresh queues so a bad
 * dashboard setting cannot prevent the two graphics from being posted.
 */
...
try {
  ...
  async function findWritableLeaderboardChannel(client) {
    ...
    if (!guild) {
      throw new Error('Guild Discord non trovata per cercare il canale classifica.');
    }
  ...
  async function directSpawnOfficialGraphics(client, reason) {
    const state = require('./bot/state');
    const lifecycle = require('./bot/lifecycle');
  ...
  if (Client && Client.prototype && !Client.prototype.__rodaLeaderboardAutoSpawnPatched) {
    ...
    Client.prototype.emit = function rodaLeaderboardAutoSpawnEmit(eventName, ...args) {
      ...
      if (eventName === 'ready' && !this.__rodaLeaderboardAutoSpawnStarted) {
        ...
        setTimeout(() => {
          directSpawnOfficialGraphics(this, 'bot_ready').catch(error => {
