...
/**
 * Creates and maintains one private text result room per registered team.
...
function isClientModuleRequest(request) {
  return typeof request === 'string' && (
    request === './bot/client' || request === './client' || request.endsWith('/bot/client') || request.endsWith('bot/client.js')
  );
}

function isPanelsModuleRequest(request) {
  return typeof request === 'string' && (
    request === './bot/panels' || request === './panels' || request.endsWith('/bot/panels') || request.endsWith('bot/panels.js')
  );
}

function getTeamEntries() {
  const state = require('./bot/state');
  return Object.entries(state.teams || {}).sort((a, b) => Number(a[1]?.slot || 9999) - Number(b[1]?.slot || 9999));
}

function getRegistrantId(teamData) {
  return clean(teamData?.registeredById || teamData?.ownerDiscordId || teamData?.captainDiscordId || teamData?.createdByDiscordId || teamData?.discordUserId || teamData?.registrantId);
}
...
function createResultPanelPayload(teamName, teamData) {
  ...
  const statusText = alreadySent
    ...
    : 'Premi il pulsante, inserisci le kill e poi invia lo screenshot/foto qui in questa stanza privata.';
  ...
  const embed = new EmbedBuilder()
    ...
    .setFooter({ text: `Stanza privata team • Match ${matchNumber}` });
...
async function findExistingPrivateChannel(guild, categoryId, teamName, slot, registrantId) {
  ...
  const marker = `risultati-${Number(slot || 0)}-`;
  ...
  return channels.find(ch => {
    ...
    return name.includes(marker) || name.includes(`risultati-${safeTeam}`) || (registrantId && name.endsWith(registrantId.slice(-4)));
  }) || null;
}
...
async function getTournamentCategory(guild) {
  ...
  const { TOURNAMENT_CATEGORY_NAME } = require('./bot/lifecycle');
  ...
  await guild.channels.fetch();
  return guild.channels.cache.find(ch => ch.type === ChannelType.GuildCategory && ch.name === TOURNAMENT_CATEGORY_NAME) || null;
}
...
async function ensurePrivateTeamChannel(client, teamName, teamData, options = {}) {
  ...
  if (!registrantId) {
    console.warn(`[team-private] impossibile creare stanza privata per ${teamName}: registrantId mancante`);
    return { ok: false, skipped: true, reason: 'missing_registrant_id' };
  }

  const existing = teamData?.privateTextChannelId
    ? await guild.channels.fetch(teamData.privateTextChannelId).catch(() => null)
    : await findExistingPrivateChannel(guild, category?.id || '', teamName, slot, registrantId);
  ...
  const channelName = `risultati-${slot || 'x'}-${safeChannelPart(teamName)}-${registrantId.slice(-4)}`.slice(0, 95);
  const overwrites = [
  ...
  if (botMember) {
    overwrites.push({ id: botMember.id, allow: [PermissionFlagsBits.ViewChannel, PermissionFlagsBits.SendMessages, PermissionFlagsBits.AttachFiles, PermissionFlagsBits.ReadMessageHistory, PermissionFlagsBits.ManageMessages, PermissionFlagsBits.ManageChannels] });
  }
  ...
  const channel = existing || await guild.channels.create({
    ...
    reason: `Stanza privata risultati RØDA CUP per ${teamName}`
  });
  ...
  lifecycle.logAudit(clean(options.registrantTag) || 'bot', 'discord', 'stanza_privata_team_assicurata', { team: teamName, slot, channelId: channel.id, registrantId });

  return { ok: true, channel, created: !existing };
}
...
async function upsertPrivateResultPanel(channel, teamName, teamData) {
    ...
    for (const message of messages.values()) {
      ...
      for (const row of message.components || []) {
        for (const component of row.components || []) {
          if (component.customId === customId) existing = message;
        }
      }
    }
  } catch {}
  ...
  const sent = await channel.send({ content: '🔒 **Stanza privata risultati team**', ...payload });
...
function installClientHook() {
  ...
  Module._load = function teamPrivateClientLoad(request, parent, isMain) {
    ...
    if (isClientModuleRequest(request) && loaded?.client && !loaded.client.__rodaTeamPrivateReadyHooked) {
      ...
      loaded.client.once('ready', () => {
        setTimeout(() => ensureAllPrivateTeamChannels(loaded.client).catch(err => console.error('[team-private] ensure all failed:', err.message)), 9000);
      });
      console.log('✅ Hook stanze private team installato.');
    }
    return loaded;
  };
}
      ...
      if (typeof originalRefresh === 'function') {
        loaded.refreshTeamResultPanels = async function patchedRefreshTeamResultPanels(...args) {
          ...
          try {
            ...
            if (client?.isReady?.()) await ensureAllPrivateTeamChannels(client);
          } catch (error) {
            console.error('[team-private] refresh privato fallito:', error.message);
          }
          return result;
        };
      }
      Object.defineProperty(loaded, '__rodaTeamPrivatePanelsPatched', { value: true, enumerable: false });
      console.log('✅ Refresh pannelli privati team collegato.');
    }
    return loaded;
  };
}
...
function installRegistrationHook() {
    ...
    if (isRegisterModal) {
      ...
      setTimeout(async () => {
        try {
          ...
          console.error('[team-private] creazione post-registrazione fallita:', error.message);
        }
      }, 2500);
    }
  ...
  console.log('✅ Creazione stanze private alla registrazione attiva.');
}

function installTeamPrivateChannels() {
  installClientHook();
  installPanelsHook();
...
module.exports = {
  installTeamPrivateChannels,
  ensurePrivateTeamChannel,
  ensureAllPrivateTeamChannels
};
