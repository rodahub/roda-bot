...
function safeChannelPart(value) {
  return clean(value).normalize('NFD').replace(/[\u0300-\u036f]/g, '').toLowerCase().replace(/[^a-z0-9]+/g, '-').replace(/^-+|-+$/g, '').slice(0, 42) || 'team';
}
...
function createResultPanelPayload(teamName, teamData) {
  ...
  const statusText = alreadySent ? (record.status === 'in_attesa' ? 'Risultato già inviato e in attesa dello staff.' : 'Risultato già registrato per questo match.') : 'Premi il pulsante, inserisci le kill e poi invia lo screenshot/foto qui in questa stanza privata.';
...
async function getTournamentCategory(guild) {
  ...
  const created = await guild.channels.create({ name: categoryName, type: ChannelType.GuildCategory, reason: 'Creazione categoria ufficiale RØDA CUP per stanze team private' });
  state.data.botSettings = state.data.botSettings || {};
...
async function ensurePrivateTeamChannel(client, teamName, teamData, options = {}) {
  ...
  const channel = existing || await guild.channels.create({ name: channelName, type: ChannelType.GuildText, parent: category.id, permissionOverwrites: overwrites, reason: `Stanza privata risultati RØDA CUP per ${teamName}` });
  if (existing) {
  ...
  lifecycle.logAudit(clean(options.registrantTag) || 'bot', 'discord', 'stanza_privata_team_assicurata', { team: teamName, slot, channelId: channel.id, categoryId: category.id, registrantId });
    ...
    if (isPanelsModuleRequest(request) && loaded && !loaded.__rodaTeamPrivatePanelsPatched) {
      ...
      if (typeof originalRefresh === 'function') {
        loaded.refreshTeamResultPanels = async function patchedRefreshTeamResultPanels(...args) {
          ...
          catch (error) { console.error('[team-private] refresh privato fallito:', error.message); }
          return result;
        };
      }
  ...
  Client.prototype.emit = function teamPrivateRegistrationEmit(eventName, ...args) {
    ...
    if (isRegisterModal) {
      ...
      setTimeout(async () => {
        ...
        } catch (error) { console.error('[team-private] creazione post-registrazione fallita:', error.message); }
      }, 2500);
    }
    return result;
  };
