 ...
 * normalizes match/pending data used by the admin dashboard, and stabilizes
 * Discord result interactions.
 */
...
const FINAL_TEAM_STATUSES = new Set(['approvato', 'inserito_manualmente', 'assente']);
...
function normalizeAdminDashboardData(data) {
  ...
  for (const [id, entry] of Object.entries(data.pending)) {
    try {
      data.pending[id] = normalizeResultEntry(entry, currentMatch);
    } catch (error) {
      console.error('[startup-guard] Pending non valido rimosso:', id, error.message);
  ...
  for (const [matchKey, match] of Object.entries(data.matches)) {
    ...
    for (const [teamName, teamState] of Object.entries(match.teams)) {
      ...
      if (teamState.status === 'approvato' || teamState.status === 'inserito_manualmente') {
        teamState.points = calcFixedPoints(teamState.placement, totalKills);
      } else if (teamState.status === 'assente') {
        teamState.points = 0;
      } else {
        teamState.points = Number(teamState.points || 0);
      }
    }
  }
  ...
  for (const [key, record] of Object.entries(data.resultSubmissions)) {
    ...
    if (record.pendingId && data.pending[record.pendingId]) {
      record.matchNumber = data.pending[record.pendingId].matchNumber;
    }
  }
...
function isPanelsModuleRequest(request) {
  return typeof request === 'string' && (request === './bot/panels' || request === './panels' || request.endsWith('/bot/panels') || request.endsWith('bot/panels.js'));
}
...
function isHelpersModuleRequest(request) {
  return typeof request === 'string' && (request === './bot/helpers' || request === './helpers' || request.endsWith('/bot/helpers') || request.endsWith('bot/helpers.js'));
}
...
function installFixedPointsGuard(helpersModule) {
  ...
  helpersModule.loadPointsConfig = function loadFixedPointsConfig() {
    return { kill: FIXED_POINTS.kill, placement: { ...FIXED_POINTS.placement } };
  };
  helpersModule.calcPoints = function guardedCalcPoints(pos, kills) {
    return calcFixedPoints(pos, kills);
  };
...
function isSubmissionsModuleRequest(request) {
  return typeof request === 'string' && (request === './bot/submissions' || request === './submissions' || request.endsWith('/bot/submissions') || request.endsWith('bot/submissions.js'));
}
...
function installSubmissionGuard(submissionsModule) {
  ...
  if (typeof submissionsModule.createPendingSubmission === 'function') {
    ...
    submissionsModule.createPendingSubmission = async function guardedCreatePendingSubmission(entry, ...args) {
      const safeEntry = normalizeResultEntry(entry, Number(entry?.matchNumber || state.data?.currentMatch || 1));
...
function isStorageModuleRequest(request) {
  return typeof request === 'string' && (request === './storage' || request === '../storage' || request.endsWith('/storage') || request.endsWith('storage.js'));
}
...
function installStorageGuard(storageModule) {
  ...
  if (typeof storageModule.loadData === 'function') {
    ...
    storageModule.loadData = function guardedLoadData(...args) {
      return normalizeAdminDashboardData(originalLoadData.apply(this, args));
    };
  }
      ...
      try {
        if (interaction && typeof interaction.isButton === 'function' && interaction.isButton() && /^(ok|no)_/.test(String(interaction.customId || ''))) {
          ...
          if (!interaction.deferred && !interaction.replied && typeof interaction.deferUpdate === 'function') {
            interaction.deferUpdate().catch(error => console.error('[startup-guard] defer bottone staff fallito:', error.message));
          }
