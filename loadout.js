...
function startLoadoutSync(){
  ...
  syncStatus = { running:true, ok:false, startedAt:nowIso(), finishedAt:null, exitCode:null, lastLines:['Avvio sync CODMunity mobile...'], error:'' };
  saveSyncStatus();
  ...
  syncProcess.on('close', code => { syncStatus.running=false; syncStatus.ok=code===0; syncStatus.exitCode=code; syncStatus.finishedAt=nowIso(); pushSyncLine(code===0?'Sync CODMunity mobile completato.':`Sync CODMunity mobile terminato con codice ${code}.`); syncProcess=null; saveSyncStatus(); });
...
function registerLoadoutRoutes(app){
  ...
  app.get('/api/admin/loadout/sync-report', (req,res)=>res.json({
    ok:true,
    codmunityMobileReport: readJSON('loadout-codmunity-mobile-sync-report.json',{}),
    codmunityDiscoveryReport: readJSON('codmunity-discovery-report.json',{}),
    manualSyncStatus: syncStatus,
    legacyReports: {
      wzloadReport: readJSON('loadout-wzload-sync-report.json',{}),
      fpsmetaReport: readJSON('loadout-fpsmeta-sync-report.json',{}),
      warzoneLoadoutReport: readJSON('loadout-warzoneloadout-sync-report.json',{})
    }
  }));
  app.get('/api/admin/loadout/sync-status', (req,res)=>{ if(!checkSyncSecret(req,res)) return; res.json({ok:true,status:syncStatus}); });
  app.get('/api/admin/loadout/run-sync', (req,res)=>{ if(!checkSyncSecret(req,res)) return; const r=startLoadoutSync(); res.json({ok:true,message:r.alreadyRunning?'Sync già in corso.':'Sync CODMunity mobile avviato.',...r}); });
  app.post('/api/admin/loadout/run-sync', (req,res)=>{ if(!checkSyncSecret(req,res)) return; const r=startLoadoutSync(); res.json({ok:true,message:r.alreadyRunning?'Sync già in corso.':'Sync CODMunity mobile avviato.',...r}); });
